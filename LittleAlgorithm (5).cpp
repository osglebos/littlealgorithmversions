...
const unsigned int ARRAY_SIZE = 8 + 1;
...
int **Inicialize(void)
{
	int tmp[ARRAY_SIZE][ARRAY_SIZE] = {
		...
		{2, 5, MAX_INF, 12, 16, MIN_INF, MIN_INF, MIN_INF, MIN_INF},
		{3, MIN_INF, 12, MAX_INF, 53, MIN_INF, 50, MIN_INF, 14},
		{4, MIN_INF, 16, 53, MAX_INF, 33, MIN_INF, 25, MIN_INF},
		{5, MIN_INF, MIN_INF, MIN_INF, 33, MAX_INF, 61, MIN_INF, MIN_INF},
		{6, MIN_INF, MIN_INF, 50, MIN_INF, 61, MAX_INF, 87, 73},
		{7, MIN_INF, MIN_INF, MIN_INF, 25, MIN_INF, 87, MAX_INF, MIN_INF},
		{8, 10, MIN_INF, 14, MIN_INF, MIN_INF, 73, MIN_INF, MAX_INF}
...
int **GetMatr(int **mas, int a, int b, int size) {
	...
	int new_inf_x = -1, new_inf_y = -1;

	for (int i = 0; i < size - 1; i++)
	{
		if (result[0][i] == mas[a][0])
			new_inf_x = i;
	}

	for (int j = 0; j < size - 1; j++)
	{
		if (result[j][0] == mas[0][b])
			new_inf_y = j;
	}

	if ((new_inf_x != -1) && (new_inf_y != -1))
		result[new_inf_y][new_inf_x] = MAX_INF;
